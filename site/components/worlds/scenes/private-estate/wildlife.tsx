...
export function Planting({ high, incident = false }: { high: boolean; incident?: boolean }) {
  ...
  useLayoutEffect(() => {
    ...
    for (let i = 0; i < grassCount; i++) {
      ...
      if (x < -5.6 && z > 5.2) x += 12
      if (incident && x > -10.6 && x < 6.8 && z < 11.9) x = 8.5 + seeded(i + 20) * 3
    ...
    for (let i = 0; i < 90; i++) {
      ...
      if (incident && i <= 45) { x = -11.5 + seeded(i + 15) * .6; z = 3.5 + seeded(i + 16) * 7 }
  ...
  }, [high, count, grassCount, incident])
...
export function WildlifeWorld({ clock, layers, quality }: WorldProps) {
