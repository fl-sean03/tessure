...
import { DataTexture, Group, Mesh, Vector3 } from 'three'
import type { WorldProps } from '../../contract'
import { attendantWalk, guestWalk, walkerRig } from './walkers'
...
export function Walker({ clock, staff, body, shoe, m, contact }: { clock: WorldProps['clock']; staff?: boolean; body: Part[]; shoe: Part[]; m: Materials; contact: DataTexture }) {
  ...
  useFrame(() => {
    const pose = (staff ? attendantWalk : guestWalk).pose(clock.current.time)
  ...
  return <group name={staff ? 'attendant-rig' : 'guest-rig'}>
    <group ref={root} name={staff ? 'attendant-body' : 'guest-body'}><Parts parts={body} />
      {[0, 1].map(side => <group key={side} ref={o => { arms.current[side] = o }} position={[side ? .215 : -.215, 1.25, 0]}>
        <mesh position={[0, -.12, 0]} material={m.skin} castShadow><capsuleGeometry args={[.055, .24, 2, 6]} /></mesh>
        <mesh position={[0, -.34, .04]} rotation={[-.22, 0, 0]} material={m.skin} castShadow><capsuleGeometry args={[.049, .15, 2, 6]} /></mesh>
      </group>)}
    </group>
