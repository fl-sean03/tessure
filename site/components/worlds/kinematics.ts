...
/** Pure authored motion; callers own foot placements, support/collision checks and rendered rigs.
 * No function reads a clock or accumulates frame state.
 * IK needs positive bone lengths, a reachable noncoincident target and a pole off the limb axis.
 * Paths need at least two distinct points with nonzero tangents; unwrap returned yaw before blending across pi.
 * Walking speed needs end > start and 0 < ramp < (end - start) / 2.
 */
